...
class Content(tk.Frame):
    ...
    def body(self):
        ...
        self.popup_menu.add_command(
            ...
            command=lambda: self.parent.controller.edit_selected_data(mode="delta"),
        )
        self.popup_menu.add_command(
            label="Ausgewählte editieren via Datum",
            command=lambda: self.parent.controller.edit_selected_data(mode="date"),
        )
        self.popup_menu.add_command(
            label="Gesamte aktuelle Tablle editieren",
            command=lambda: self.parent.controller.edit_selected_data(
                mode="delta", all=True
            ),
        )
        self.popup_menu.add_command(
            label="Änderungen für gesamte Tabelle zurücknehmen",
            command=lambda: self.parent.controller.rollback_selected_data(),
        )
        self.popup_menu.add_command(
            label="Änderungen für gesamte Tabelle speichern",
        ...
        self.addi_popup_menu.add_command(
            label="Ausgewählte editieren via Delta",
            command=lambda: self.parent.controller.edit_selected_data(
                mode="delta", infoview=True
            ),
        )
        self.addi_popup_menu.add_command(
            label="Ausgewählte editieren via Datum",
            command=lambda: self.parent.controller.edit_selected_data(
                mode="date", infoview=True
            ),
        )
        self.addi_popup_menu.add_command(
            label="Gesamte aktuelle Tablle editieren",
            command=lambda: self.parent.controller.edit_selected_data(
                mode="delta", all=True, infoview=True
            ),
        )
        self.addi_popup_menu.add_command(
            label="Änderungen für gesamte Tabelle zurücknehmen",
    ...
    def fill_info_section(self, data):
        ...
        for info in data:
            if data[info]:
                ...
                headinglist = [
                    attr.name for attr in data[info][0].attr_list if data[info]
                ]
                ...
                for item in data[info]:
                    ...
                    try:
                        ...
                        values.append("None")
                        values.append(0)
                    insert = infoview.insert(
                        "", "end", text=item.attr_list[0].value, values=tuple(values)
                    )
                ...
                self.info_views[info] = [
                    infoview,
                    str(data[info][0].__class__.__name__) + "Handler",
                ]
            else:
                tab = ttk.Frame(self.tab_control)
                self.tab_control.add(tab, text=info)
    ...
    def fillHistoryData(self, history_data):
        ...
        for entry in history_data:
            ...
            parent = self.dataview.insert(
                "",
                "end",
                text=entry.place.url,
                values=(
                    v_date,
                    lv_date,
                    str(entry.__class__.__name__) + "Handler",
                    entry.id,
                ),
            )
